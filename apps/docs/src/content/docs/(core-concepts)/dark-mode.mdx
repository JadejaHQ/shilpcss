...
Shilp CSS supports **class-based dark mode by default**, with full control over
how it is applied and customized.
...
`@theme dark-any { ... }` is not included by default. you have to explicitely
add this. Light variants are also available.
...
Class-based theming is recommended. Becuase, it keeps selectors simple and it
minimizes output size.

Using data-theme widely can increase CSS size if applied inconsistently across
the app.
...
Parent-based dark mode is best suited when we need site-wide theming. A single
source of truth, controls the entire interface.
...
This allows isolated components to opt-into dark mode without affecting the
entire page.
