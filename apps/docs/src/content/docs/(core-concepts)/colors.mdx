...
[Tailwind CSS](https://tailwindcss.com/docs/colors) and is suitable for a wide
range of design styles, minimal, expressive, or system-heavy.
...
Compared to `hex`, `rgb`, or `hsl`, `oklch` simply produces colors that _look
right_, not just mathematically calculated right.
...
Shilp CSS also includes **OKLCH equivalents** for all native HTML named colors,
such as orange, white, green, blue, gold, etc.

This allows you to use familiar color names. They behave like the rest of the
system, predictable, themeable, and token-driven.
...
To ensure predictable behavior across all supported browsers, at build time,
Shilp CSS uses **Lightning CSS** to transforms these values.

This means:

1. `oklch` to `lab` format conversion for internal processing
   - `lab` is a simpler Cartesian form, easier to clamp and serialize, and more
...
The scale gives you enough range to express hierarchy without inventing custom
values.
...
Shilp CSS strongly recommends styling with **brand and semantic design tokens**,
not just raw color values.
...
See: [Brand and Semantic Color Reference](#brand-and-semantic-colors-reference)
...
See: [Color Roles Reference](#color-roles-reference)
...
`theme(...)` always resolves during build. No runtime evaluation happens post
build.

</Alert>

See:
[Inline Theme Function Configuration](/docs/config/theme#inline-theme-function)
...
[`shilp.config.js`](/docs/config/shilp-config-file).
