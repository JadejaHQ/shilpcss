...
With Shilp CSS, you don’t build styles by accumulating utilities in HTML, You
compose them in CSS by clearly stating what an _element is responsible for_.

<Alert>

_If any concept or syntax here feels unfamiliar, don’t worry. You can safely
skip it for now. Everything is explained gradually in later chapters._
...
Shilp CSS does not invent a special syntax for everything. If something does not
fit an intent, mixin or an utility, **use plain CSS**.
...
This keeps custom CSS aligned with the system without forcing everything through
intent and utilities.
...
**If you reach for it often, it usually means intent boundaries are breaking
down.** _This is where chaos begins._
...
wins**.<br /> For simplicity, ASSUME the last declration wins.

This follows normal CSS behavior. Shilp CSS does not add conflict resolution
logic on top.
...
See:
[Understanding Specificity](/docs/best-practices/writing-shilp-css#understanding-specificity)
...
_Shilp CSS keeps Intent and Utility-first idea both_, and makes sure they live
where they belong.
...
You **CAN NOT USE** names reserved by `CSS` or `SCSS`. Using them **WILL CAUSE
CONFLICTS**.
