...
It is designed for **modern browsers only**, with no legacy fallbacks, no hidden
polyfills, and **no bleeding-edge features, while respecting the baseline**.
...
This baseline is enforced using **Lightning CSS** and by carefully choosing
which selectors (mixins), properties, and values.
...
Some features are intentionally restricted, even if they are partially supported
across browsers.
...
These are not oversights. They are conscious choices to avoid undefined
behavior.
...
- **Lightning CSS** then applies Minification, Vendor prefixes and Safe
  fallbacks

Because **Lightning CSS** may add small fallback rules, the final output can
include CSS that did not exist before purging.

As with any purge system, 100% unused CSS removal is not guaranteed. This is a
known and accepted tradeoff.
...
If you project requires Legacy browser support, Automatic polyfills or Backward
compatibility guarantees, Shilp CSS's default config may not be the right fit.
...
If you are building for modern browsers and you value clarity over speed, this
baseline will feel natural.
