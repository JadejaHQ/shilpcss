...
_Naming things is hard, Fear is real for broken styles by changing just one line
and avoiding it entirely is one reason utility-first (atomic classes) approaches
feel appealing._

This page guides you how to write, name, and organize Shilp CSS.

Not theory. <br /> Not trends. <br /> Just how to work with Shilp CSS in a way
...
Utility-first systems reduce naming. But, long-lived systems always bring naming
back.
...
When layout changes, you do not touch the component. This is how refactors stay
calm.
...
If you write CSS, you must understand specificity. It is part of writing safe
CSS.

Specificity is a way to help browser decide, _when two CSS rules try to style
the same thing, which one wins?_
...
in Shilp CSS, you can add `!important` flag to any property just by appending
`!` to the utility.
...
Spend one hour learning through below resources properly. It will remove 80% of
CSS frustration.
...
Refactor is predictable. Because Shilp CSS uses static class names, search
works.
...
> Shilp CSS fixes grouping (via intents). <br /> You fix naming (via clear
> roles).
...
The button we are about to build is **not a production-ready design system
button**.
...
Once you understand the structure, you can build buttons as complex as your
system requires.
...
Notice something important: <br /> We never changed `.button`. We added
...
`has-icon` class on button is needed as `:has` is not supported by Shilp CSS
baseline, **yet**.

See:
[Deliberate Limitations](/docs/browser-compatibility#deliberate-limitations)
...
One place (_practically some_), Not multiple markup files. This is the
advantage.
