...
Sometimes one utility should apply multiple properties, but all resolve to the
same value.
...
Some CSS features supports **full shorthand property** and also **individual
sub-properties**.

For example: `animation`, `transition`, `flex`

For this, you can combine a **preset utility** with detailed individual
utilities.
...
Most CSS properties in real-world usage do not require multi-part composition.
Overriding is predictable and simpler.
...
Some CSS systems are easier to manage using **CSS variables as building
blocks**.
