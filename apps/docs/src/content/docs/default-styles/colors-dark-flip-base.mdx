...
Instead of manually redefining each color role, this **file flips the base
(neutral) color scale** used across the system.
...
Because many UI variables rely on the base scale, the interface automatically
adapts to dark mode.
...
Using `colors-dark-flip-base.css`, instead of redefining roles, the **entire
base palette flips**.
...
This is faster and works well when your UI heavily relies on base (neutral)
scales.
