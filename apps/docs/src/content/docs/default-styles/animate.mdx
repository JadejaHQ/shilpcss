...
Like other parts of Shilp CSS, the system relies heavily on **CSS variables** so
utilities can compose motion instead of replacing it.
...
This file supplies the motion primitives used by animation and transition
utilities.
...
These primitives allow animation utilities to produce consistent motion across
the UI.
...
These values are used by animation and transition utilities so motion across the
system follows the same curves.
...
Instead of repeating long lists of properties, this file defines reusable
groups.
...
Instead of defining many animation variants, Shilp CSS composes animations using
variables.

Variables describe the initial and / or final state of an animation. This can be
composed as **fade**, **slide**, **zoom** for enter and exit individually.

<Alert variant="info">

The enter and exit keyframes are adapted from
[`tailwindcss-animate` Package](https://www.npmjs.com/package/tailwindcss-animate)
to provide a composable animation system.
...
In most cases, customization should happen through **variables**, not style
rules.
