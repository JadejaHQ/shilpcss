...
Browsers apply many built-in styles to elements. These defaults vary between
browsers and can lead to inconsistent layouts.
...
Some browsers fake bold or italic styles when the font doesn’t provide them.
This reset prevents those synthetic styles.
...
This reset prevents layout shifts by fixing the html viewport and moving scroll
behavior to body.
...
Native browser styles such as button backgrounds and input decorations are
removed.
...
However, some third-party libraries assume browser defaults are untouched. In
those cases, small scoped overrides may be required.
...
Libraries that render nodes, edges, or canvases often rely on default SVG
behavior.
...
If your editor renders Markdown or HTML content, restore typography inside the
editor scope like:
...
html, :host /* 7 */ {
	...
	line-height: 1.5; /* 3 */
	font-family:
		ui-sans-serif, sans-serif, system-ui, "Apple Color Emoji", "Segoe UI Emoji",
