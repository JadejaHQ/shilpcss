...
These are **not hardcoded**. They are regular CSS files. You can read, modify,
or replace them like any other stylesheet.

You include only what you need. This keeps the system flexible and avoids
forcing any styling decisions.
...
All of these are optional. You can include them fully, partially, or replace
...
This file **does not include dark mode colors**. It only defines the light theme
color palette.
...
Avoid writing styles directly in `style.css`, where Shilp CSS styles are
imported.
...
They provide a sensible starting point without locking you into a specific
design.
