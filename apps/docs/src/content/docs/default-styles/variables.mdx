...
variables**. The final property is then composed from them.

This design prevents utilities from overwriting each other and allows multiple
utilities to combine safely.
...
Many utilities rely on the variables defined in this file to compose their final
CSS properties.
...
Instead of writing the full CSS value every time, Shilp CSS builds those values
from smaller pieces using re-usable CSS variables.

Utilities update **individual parts**, and the final property is composed from
them.

### About Property Overrides

One benefit of this architecture is that utilities no longer overwrite each
other when working on the **same CSS property**.

However, avoiding property conflicts is **only a side effect** of the
composition system.

The main design goal is **building complex CSS values from smaller reusable
parts**.
...
Because these are used less frequently and their values vary widely, Shilp CSS
does not manage them through composition variables.
...
Web fonts may **load late or fail completely** due to network issues, blocking,
or privacy settings.

To ensure text remains readable, every font should have a **reliable fallback
stack**.
...
These variables contain **system-safe font stacks**.
...
Instead of writing the entire transform property each time, Shilp CSS stores
each part in a variable. Utilities update only the piece they control.
...
This architecture allows multiple utilities to **compose a final style
together** instead of replacing each other.
...
This pattern can be used to enable features only when the browser supports OKLCH
colors.
...
See:
[Important Note For RGB Format](/docs/config/shilp-config-file#important-note-for-rgb-format)
...
If any part of this file feels difficult to understand, copy the snippet and
paste it into [ChatGPT](https://chatgpt.com) and ask it to explain the code in
simple words.
