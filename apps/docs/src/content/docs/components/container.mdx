...
A **container** is a layout component that provides a **consistent content width
across breakpoints**.

The container width adapts as the viewport changes while keeping a predictable
layout structure. This allows pages to scale across devices while maintaining
consistent margins and readable content width.
...
If a breakpoint is removed or added, the generated container classes change
accordingly.
...
The container configurations allow you to customize the behavior and appearance
...
This ensures the container maintains **consistent margins from the viewport
edges**.

<Alert variant="warning">

The keys inside `containerConfig.spacing` **must match the breakpoint names**
defined in `shilpConfig.theme.screens`.

</Alert>

For default breakpoints and container configuration, width of the container at
each breakpoints is shown in the below table.
...
The keys inside `containerConfig.innerPadding` **must match the breakpoint
names** defined in `shilpConfig.theme.screens`.

</Alert>

For default breakpoints and container configuration, horizontal padding of the
container at each breakpoints is shown in the below table.
