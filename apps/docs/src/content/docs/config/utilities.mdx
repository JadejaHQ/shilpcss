...
If intent defines _where something belongs_, utility defines _what exactly to
apply_.
...
From the [Intents Config Page](/docs/config/intents) we know that, Shilp CSS
extracts **intent-name and list of utilities**.
...
Variant behavior only works, if the
[Properties Config](/docs/config/properties#variant) explicitly enables it.

**If not enabled**: <br /> Then `/2` in `@space -ml-1/2!;` is part of the
utility value (fraction), not a variant.

**If enabled**: <br /> Then only the first `/` is considered. Any slash `/`
...
`variant: true` is effective for **color values** only (modifies
`opacity`).<br />Other values don't have any logic to handle the variant.
...
Tokens used to reach **property config** is called as **Property Tokens** and
the remaining tokens are called **Value Tokens**.
