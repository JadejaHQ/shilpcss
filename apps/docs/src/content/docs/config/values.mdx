...
if intent decides _where_, utility decides _what_, property decides _how_, then
**values decides what the actual data is**.
...
- How `global values`, `propertyConfig.themeKey`, and `propertyConfig.values`
  are merged
...
- If `propertyConfig.resolve` is defined → use resolver from
...
For [Inline Theme Function](/docs/config/theme#inline-theme-function) with
...
Here, considering standard `font-size` on `html` is `16px`. This will becomes
`16px = 1rem`

Values provided to property config are in `px` unit and built-in values dataset
contains `px` unit mostly.
...
There is no nested objects structure like `shilpConfig.values`. just plain
...
Here, either `options.propertyConfig` (for intent) or
`options.inlineThemeConfig` (for inline theme function) will be available.
...
3. Extend configs are deep merged
   ...
   - `shilpConfig.extend.valueResolvers` deep merged with
     `shilpConfig.valueResolves`
...
You can explore the [full list](/docs/values) in the **VALUES** section of the
documentation.
