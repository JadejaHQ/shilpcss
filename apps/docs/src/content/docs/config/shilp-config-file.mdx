...
Below is the full shape of the config file. _Almost_ everything is just nested
objects.
...
Root-level config object, overrides internal default config completely. This is
a full override.
...
In most cases, extend is safer. Use root-level replacement only when you want
full control.
...
For example: [Mixins](/docs/config/mixins) uses `list` module from SCSS so it is
added (`@use "sass:list";`) to the `imports` internally by default.

<Alert variant="danger">

While adding imports, don't forget to end each import statement with colon
(`;`). Internally it is used to split all the imports and then only unique
imports are passed to SCSS compiler.
...
In the above table `options` is argument passed to the
`shilpConfig.scss.options` function. don't get confused.
...
No Shilp CSS default options, configured as of now. SCSS's internal
...
If you define `scss.options` config, Your returned object becomes final **SCSS**
config.
...
This is the **only mandatory option** Shilp CSS expects to add purging
functionality.
...
It defines which markup files are scanned for removing unused styles
(_purging_).
...
By default, Shilp CSS recommends scanning build output folders for more accurate
and efficient purging.
...
If you define `purge` config, Your returned object becomes final **Purge CSS**
config.
...
There is known issue related to Purge CSS integration specifically for server
rendered apps like Next or Nuxt.

This will be addressed in future releases (before the **v1.0.0 stable
release**).
...
All the colors authoring happens in OKLCH format irrespecitve of the value of
this option.
...
Shilp CSS uses an **isolated Lightning CSS instance with a lower browser
baseline** to safely transform the color format.
...
A function that receives the final **Lightning CSS** options for styles
optimization.

<Alert variant="info">

Transpilation process runs **after the build generation**. So, if you apply some
config and it is not being reflected in development environment (local dev
server), It is the expected behaviour.
...
You can modify or replace the options before they are passed to **Lightning
CSS**.

<Alert variant="danger">

If you define `transpile` config, Your returned object becomes final **Lightning
CSS** config.
...
This happens inside the development environment. It is simply build-time
processing happening live.
