...
If **intents** are about “_What styles to apply?_” then **mixins** are about
“_Under what conditions to apply those styles?_”
...
Internally, Shilp CSS generates SCSS `@mixin` just-in-time, moves it to the top
of the file, and replaces Shilp CSS mixin with an `@include`.
...
If the mixin name does not exist in config, but used in `.css`, then it will not
be processed.
...
If an argument contains a comma, it will otherwise be split into multiple
arguments:
...
time. <br /> If argumetns number did not match, SCSS compiler will throw error.

<Alert variant="info">

**Recommendation**: _Always wrap function variant arguments in quotes. Even if
they don’t contain commas._
...
- `definition` (string) - SCSS's full `@mixin` definition
  - This is optional
  - It must follow
    [SCSS `@mixin` Pattern](https://sass-lang.com/documentation/at-rules/mixin)
- `selector` (string) - Final selector to replace Shilp CSS mixin syntax
  ...
  - If `definition` not provided, then it must be resolved selector (valid `CSS`
    selector)
...
Mixin processing happens during build generation. There is no runtime
evaluation.
...
You **CAN NOT USE** following names reserved by `CSS` or `SCSS`. Using these as
intents or mixins names, **WILL CAUSE CONFLICTS**.
...
You can explore the [full list](/docs/mixins) in the **MIXINS** section of the
documentation.
