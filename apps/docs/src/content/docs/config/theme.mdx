...
If values are _raw primitives_ and properties _consume values_, then **theme
organizes values so they can be reused consistently**.
...
List of [Available Internal Theme Dataset](#available-built-in-theme-dataset) is
...
Example: <br /> `color: theme(colors-orange-600);` →
...
For inline theme `options.propertyConfig` will be undefined and
`options.inlineThemeConfig` will be available.
...
Inline theme is processed during build generation. There is no runtime
evaluation.
...
To avoid this case, Shilp CSS scans and resolves inline theme functions early
and outputs plain css value.
...
As inline theme function is not resolved during runtime, if not processed at
this stage, browser would not undertand this and styles were not applied.
